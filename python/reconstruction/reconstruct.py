                                                                         ...
                                                                         for iExp in np.arange(self.nExp):
                                                                             ...
                                                                             dx = xsample-xcen
                                                                                 dy = ysample-ycen
                                                                                     ...
                                                                                     dd[:, 1] = dy.flatten()
                                                                                     if (iExp < self.nExp):
                                                                                         kernelvalue = self._kernel(self.fwhm[iWave,iExp]*alpha)* (self.dimage / self.dkernel)**2
                                                                                             else:
                                                                                                 kernelvalue = self._kernel(np.mean(self.fwhm[iWave,:])*alpha)* (self.dimage / self.dkernel)**2
                        ...
                        return(wwT)
                            ...
                            def set_cube(self,**kwargs):
                                """Set cube for each wavelength
                                    ...
                                    [nside, nside, nWave]
                                    """
                                        self.cube_psf,self.cube_psf_ivar,self.cube_psf_corr,self.cube_psf_mask =self.calculate_cube(self.flux_psf,self.flux_psf_ivar,self.flux_psf_mask,**kwargs)
                                        self.cube,self.cube_ivar,self.cube_corr,self.cube_mask = self.calculate_cube(self.flux,self.flux_ivar,self.flux_mask,**kwargs)
                                        return
                                            
                                            def calculate_cube(self, flux, flux_ivar,flux_mask,**kwargs):
                                                    ...
                                                    """
                                                        cube = np.zeros([self.nside, self.nside, self.nWave],
                                                                        ...
                                                                        cube_ivar = np.zeros([self.nside, self.nside, self.nWave],
                                                                            ...
                                                                            i=0
                                                                                cube_corr=[]
                                                                                cube_mask = np.zeros([self.nside, self.nside, self.nWave],
                                                                                                     dtype=np.int32)
                                                                                                     for iWave in np.arange(self.nWave):
                                                                                                                                                                                           ...
                                                                                                                                                                                           if iWave in self.bbindex:
                                                                                                                                                                                               ...
                                                                                                                                                                                               i=i+1
                                                                                                                                                                                                       ...
                                                                                                                                                                                                       i=i+1
                                                                                                                                                                                                           ...
                                                                                                                                                                                                           # mask
                                                                                                                                                                                                           cube_mask[:,:,iWave]=self.mask(iWave,flux_ivar,flux_mask,w0).reshape([self.nside,self.nside])
                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                       """Return cube covariance matrix for a wavelength
                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                           """
                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                               return(covar)
                                                                                                                                                                                                                                   ...
                                                                                                                                                                                                                                   def mask(self,iWave=None,flux_ivar=None,flux_mask=None,weights=None):
                                                                                                                                                                                                                                       """Return mask matrix for a typical wavelength given the weights matrix
                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                           """
                                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                                               index_nocov= np.where(weights.sum(axis=0)==0)[0]
                                                                                                                                                                                                                                                                                                                                          ...
                                                                                                                                                                                                                                                                                                                                          def set_band(self):
                                                                                                                                                                                                                                                                                                                                              """ set average result for each band simulation and RSS and its FWHM
                                                                                                                                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                                                                                                                                  """
                                                                                                                                                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                                                                                                                                                      self.GFWHM = self.FWHM(self.xi, self.yi, self.GPSF)[0]
                                                                                                                                                                                                                                                                                                                                                      self.RFWHM = self.FWHM(self.xi, self.yi, self.RPSF)[0]
                                                                                                                                                                                                                                                                                                                                                      self.IFWHM = self.FWHM(self.xi, self.yi, self.IPSF)[0]
                                                                                                                                                                                                                                                                                                                                                      self.ZFWHM = self.FWHM(self.xi, self.yi, self.ZPSF)[0]
                                                                                                                                                                                                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                                                                                                                                                                                                      dy = ys
                                                                                                                                                                                                                                                                                                                                                                                                          ...
                                                                                                                                                                                                                                                                                                                                                                                                          pars = [1, 2]
                                                                                                                                                                                                                                                                                                                                                                                                              ...
                                                                                                                                                                                                                                                                                                                                                                                                              rgrid = np.arange(100000) / 100000. * 5.
                                                                                                                                                                                                                                                                                                                                                                                                                  imagewidth = rgrid[np.argmin(np.abs(self.gaussian(rgrid, kernel_par[0]).max() / 2 - self.gaussian(rgrid, kernel_par[0])))] * 2
...
def set_base(plate=None, ifu=None, release='MPL-5', waveindex=None,dkernel = 0.05):
...
def set_G(plate=None, ifu=None, release='MPL-5', waveindex=None,dimage = 0.75,lam=0,alpha=1,beta=1,xcen=None,ycen=None,noise=None):
...
def set_Shepard(plate=None, ifu=None, release='MPL-5', waveindex=None,dimage = 0.75,alpha=1,beta=1,xcen=None,ycen=None,noise=None):
...
def write(datafile, filename):
    ...
    def table_correlation(correlation=None,thresh=1E-14):
...
if 'BZERO' not in list(cubehdr.header.keys()):
    ...
    GIMG_hdr = insert_cardlist(hdu=GIMG_hdr,insertpoint='EXTNAME',cardlist=card_WCS_list+card_loc_list+[card_GFWHM],after=False)
    
    RIMG_hdr=fits.ImageHDU(name='RIMG',data=datafile.RIMG)
    RIMG_hdr = insert_cardlist(hdu=RIMG_hdr,insertpoint='EXTNAME',cardlist=card_WCS_list+card_loc_list+[card_RFWHM],after=False)
    
    IIMG_hdr=fits.ImageHDU(name='IIMG',data=datafile.IIMG)
    IIMG_hdr = insert_cardlist(hdu=IIMG_hdr,insertpoint='EXTNAME',cardlist=card_WCS_list+card_loc_list+[card_IFWHM],after=False)
    
    ZIMG_hdr=fits.ImageHDU(name='ZIMG',data=datafile.GIMG)
    ZIMG_hdr = insert_cardlist(hdu=ZIMG_hdr,insertpoint='EXTNAME',cardlist=card_WCS_list+card_loc_list+[card_ZFWHM],after=False)
    
    GPSF_hdr=fits.ImageHDU(name='GPSF',data=datafile.GPSF)
    GPSF_hdr = insert_cardlist(hdu=GPSF_hdr,insertpoint='EXTNAME',cardlist=card_WCS_list+card_loc_list+[card_GFWHM],after=False)
    
    RPSF_hdr=fits.ImageHDU(name='RPSF',data=datafile.RPSF)
    RPSF_hdr = insert_cardlist(hdu=RPSF_hdr,insertpoint='EXTNAME',cardlist=card_WCS_list+card_loc_list+[card_RFWHM],after=False)
    
    IPSF_hdr=fits.ImageHDU(name='IPSF',data=datafile.IPSF)
    IPSF_hdr = insert_cardlist(hdu=IPSF_hdr,insertpoint='EXTNAME',cardlist=card_WCS_list+card_loc_list+[card_IFWHM],after=False)
    
    ZPSF_hdr=fits.ImageHDU(name='ZPSF',data=datafile.GPSF)
    ZPSF_hdr = insert_cardlist(hdu=ZPSF_hdr,insertpoint='EXTNAME',cardlist=card_WCS_list+card_loc_list+[card_ZFWHM],after=False)
...
hdu=fits.HDUList([hp,cubehdr,PSF_hdr,ivar_hdr,mask_hdr,
                  datafile.rss.data['WAVE'],datafile.rss.data['SPECRES'],datafile.rss.data['SPECRESD'],datafile.rss.data['OBSINFO'],
